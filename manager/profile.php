<?php
 ...
 if (isset($_POST['update_btn'])) {
  ...
  $sql = "UPDATE admins SET fullname = '$fullname', phone = '$phone', email = '$email' WHERE admin_id = $admin_id";
 ...
 if (isset($_POST['password_btn'])) {
    $current_password = CHECK_INPUT(SANITIZE($_POST['password']));
    $newpassword = CHECK_INPUT(SANITIZE($_POST['newpassword']));
    $renewpassword = CHECK_INPUT(SANITIZE($_POST['renewpassword']));

    if ($newpassword !== $renewpassword) {
        echo "<script>alert('Passwords do not match')</script>";
    } else {
      $pwd = ENCRYPT($renewpassword);
      $old_password = ENCRYPT($current_password);

      if ($old_password !== $password) {
        echo "<script>alert('Old password do not match!')</script>";
    } else {
      $sql = "UPDATE admins SET password = '$pwd' WHERE admin_id = $admin_id";
      $result = VALIDATE_QUERY($sql);
              ...
              <div class="tab-content pt-2">
                ...
                <div class="tab-pane fade show active profile-overview" id="profile-overview">
                  ...
                  <div class="row">
                    ...
                    <div class="col-lg-9 col-md-8">(+234) <?= $phone; ?></div>
                  </div>
                ...
                <div class="tab-pane fade pt-3" id="profile-change-password">
                  ...
                  <form action="" method="post">
                    ...
                    <div class="text-center">
                      <button type="submit" name="password_btn" class="btn btn-primary">Change Password</button>
                    </div>
